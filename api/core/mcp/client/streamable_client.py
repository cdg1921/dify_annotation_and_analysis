...
# cdg:定义请求上下文，包含客户端、请求头、会话ID、会话消息、元数据、服务器到客户端的队列、SSE读取超时
# cdg:@dataclass是python的装饰器，用于将类转换为数据类，如果没有@dataclass，不能直接用RequestContext.client，RequestContext.headers等
...
# cdg:StreamableHTTPTransport是StreamableHTTP客户端传输的实现
class StreamableHTTPTransport:
    ...
    # cdg:检查消息是否是初始化请求。JSONRPCMessage是联合类型，其示例对象message.root是RootModel的用法。
    ...
    def _handle_sse_event(
        ...
        if sse.event == "message":
            try:
                message = JSONRPCMessage.model_validate_json(sse.data)  # cdg:JSONRPCMessage.model_validate_json方法继承于BaseModel
    ...
    # cdg:处理意外的响应内容类型，用于处理服务器返回的意外内容类型
...
# cdg:StreamableHTTP客户端的上下文管理器，用于创建StreamableHTTP客户端的连接
@contextmanager   # cdg: @contextmanager是python的装饰器，用于将函数转换为上下文管理器
def streamablehttp_client(
    ...
    # cdg:创建StreamableHTTPTransport实例，用于创建StreamableHTTP客户端的连接
    # cdg:其中'sse_read_timeout'参数决定了客户端等待新事件的时间（以秒为单位），所有其他HTTP操作由'timeout'参数控制。
    # cdg:返回一个元组，包含：
    # cdg:- server_to_client_queue: 从服务器读取消息的队列, 即read_stream ，用于接收服务器发送的消息
    # cdg:- client_to_server_queue: 发送消息到服务器的队列, 即write_stream，用于发送消息到服务器
    # cdg:- get_session_id_callback: 获取当前会话ID的回调函数, 即get_session_id，用于获取当前会话ID
    """
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        try:
            ...
            try:
                ...
                pass
            # cdg:添加None哨兵到客户端到服务器的队列
            client_to_server_queue.put(None)
            # cdg:添加None哨兵到服务器到客户端的队列
