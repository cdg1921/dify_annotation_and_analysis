...
# cdg:定义状态准备，包含端点URL。@final是python的装饰器，用于将类转换为最终类，不能被继承
@final
class _StatusReady:
...
# cdg:定义读取队列，包含会话消息、异常、空。TypeAlias是python的类型别名，用于定义类型别名
...
# cdg:SSETransport是SSE客户端传输的实现，继承自BaseTransport，实现了SSE客户端传输的功能
class SSETransport:
    ...
    def sse_reader(self, event_source, read_queue: ReadQueue, status_queue: StatusQueue) -> None:
        ...
        Args:
            ...
            status_queue: Queue to put status updates.
        """
        # cdg:读取SSE事件，将事件添加到读取队列中,以read_queue作为函数参数，在参数中修改read_queue的内容，所以不需要返回值
        try:
    ...
    def _send_message(self, client: httpx.Client, endpoint_url: str, message: SessionMessage) -> None:
        ...
        """
        # cdg:发送单个消息到服务器，使用client.post方法，将message转换为JSON格式，并发送
        ...
        # cdg:如果没有报错就认为发送成功，如果报错就认为发送失败，并抛出异常
    ...
    def post_writer(self, client: httpx.Client, endpoint_url: str, write_queue: WriteQueue) -> None:
        ...
        """
        # cdg:写入消息到服务器，使用write_queue.get方法，获取消息，如果消息为None，则退出循环，如果消息为Exception，则将消息添加到write_queue中，继续循环，如果消息为SessionMessage，则调用_send_message方法发送消息
        # cdg:以write_queue作为函数参数，并在函数中更新write_queue的内容，所以不需要返回值
        try:
            while True:
                try:
    ...
    def connect(
        ...
        # cdg：在读和写的过程中更新read_queue和write_queue的内容，所以不需要返回值
        # cdg:但处理完成后，需要返回两个消息队列中的内容
        
        # cdg:返回读取队列和写入队列
        return read_queue, write_queue

# cdg:SSE客户端的上下文管理器，用于创建SSE客户端的连接
@contextmanager
def sse_client(
    url: str,
    headers: dict[str, Any] | None = None,
