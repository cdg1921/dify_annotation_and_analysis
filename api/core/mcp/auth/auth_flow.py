...
# cdg:定义OAuth回调状态，包含提供者ID、租户ID、服务器URL、元数据、客户端信息、代码验证器、重定向URI
...
# cdg:生成PKCE挑战和验证器，用于OAuth 2.0的授权码模式，pkce是“Proof Key for Code Exchange”的缩写，是一种OAuth 2.0的授权码模式，用于在客户端和服务器之间交换代码。
...
# cdg:创建安全的Redis状态，用于存储OAuth回调状态，使用随机状态键，并设置过期时间
...
# cdg:从Redis中检索OAuth回调状态，使用状态键，并删除状态数据
...
# cdg:处理OAuth回调，使用状态键和授权码，交换授权码，并保存令牌
...
# cdg:发现OAuth元数据，使用服务器URL，并返回OAuth元数据
...
# cdg:开始授权，使用服务器URL、元数据、客户端信息、重定向URL、提供者ID、租户ID，生成授权URL和代码验证器
...
# cdg:交换授权码，使用服务器URL、元数据、客户端信息、授权码、代码验证器、重定向URI，交换授权码，并返回令牌
...
# cdg:刷新授权，使用服务器URL、元数据、客户端信息、刷新令牌，刷新授权，并返回令牌
...
# cdg:注册客户端，使用服务器URL、元数据、客户端元数据，注册客户端，并返回客户端信息
def register_client(
    server_url: str,
    metadata: Optional[OAuthMetadata],
...
# cdg:处理OAuth授权，使用提供者、服务器URL、授权码、状态参数、是否为列表，处理OAuth授权，并返回结果 
def auth(
